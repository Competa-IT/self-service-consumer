...
class Invitation:
    ...
    def evaluate_retry(self, username: str):
        ...
        if retries > 4:
            self.logger.error(
                "Maximum retries reached for user %s. Check the UMC logs for more information",  # noqa: E501
                username,
            )
        ...
        self.logger.debug(
            "Tried sending the invitation email for %s %s times", username, retries
        )
    ...
    def run(self):
        self.logger.info(
            "Starting the filesystem watch to trigger invitation emails via the UMC"
        )
        while True:
            self.logger.debug(
                "Checking queue directory for new files: %s", self.queue_directory
            )
